...
public class HealthViewModel extends ViewModel {
    ...
    public void fetchHealthNews() {
        NewsApiService apiService = ApiClient.getClient().create(NewsApiService.class);
        Call<NewsResponse> call = apiService.getCategoryHeadlines(Constants.COUNTRY_CODE, "health", Constants.API_KEY, DEFAULT_PAGE_SIZE);

        call.enqueue(new Callback<NewsResponse>() {
            @Override
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    healthNews.postValue(response.body().getArticles());
                } else {
                    errorMessage.postValue("Error fetching health news: " + response.message());
                }
            }
