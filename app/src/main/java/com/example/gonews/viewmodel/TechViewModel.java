...
public class TechViewModel extends ViewModel {
    ...
    private final String CATEGORY = "technology"; // Kategori spesifik untuk ViewModel ini
    ...
    public void fetchTechNews(boolean isLoadMore) {
        ...
        Log.d("TechViewModel", "Fetching tech news for country: " + Constants.COUNTRY_CODE +
                ", category: " + CATEGORY +
        ...
        call.enqueue(new Callback<NewsResponse>() {
            ...
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (newsResponse.getResults() != null && !newsResponse.getResults().isEmpty()) {
                        ...
                        Log.d("TechViewModel", "Tech news fetched successfully. Total results: " + newsResponse.getTotalResults() +
                                ". Articles fetched in this call: " + newsResponse.getResults().size() +
                                ". Next Page Token: " + nextPageToken);
                    } else {
                        String msg = "Tidak ada berita teknologi ditemukan untuk " + Constants.COUNTRY_CODE + " (" + Constants.LANGUAGE_CODE + "). Ini mungkin karena batasan API Key gratis atau tidak ada berita yang tersedia.";
                        errorMessage.postValue(msg);
                        if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan tidak ada hasil
                            techNews.postValue(null);
                        }
                        Log.w("TechViewModel", msg + " Full response: " + newsResponse.toString());
                    }
                } else {
                    String errorBody = "";
                    try {
                    ...
                    String msg = "Error fetching tech news: HTTP " + response.code() + " - " + response.message() + " | " + errorBody;
                    errorMessage.postValue(msg);
                    if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan ada error
                        techNews.postValue(null);
                    }
                    Log.e("TechViewModel", msg);
                }
            }
            ...
            public void onFailure(Call<NewsResponse> call, Throwable t) {
                isLoading.postValue(false); // Selesai loading
                String msg = "Kesalahan Jaringan saat mengambil berita teknologi: " + t.getMessage();
                errorMessage.postValue(msg);
                if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan ada kegagalan
                    techNews.postValue(null);
                }
                Log.e("TechViewModel", msg, t);
            }
        });
    }
    ...
    public void resetPagination() {
        nextPageToken = null;
        techNews.postValue(null); // Kosongkan data yang ada
