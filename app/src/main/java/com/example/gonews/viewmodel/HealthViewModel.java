...
import com.example.gonews.model.Article; // Tetap menggunakan nama 'Article'
import com.example.gonews.model.NewsResponse; // Ini adalah model respons NewsData.io yang baru
...
public class HealthViewModel extends ViewModel {
    ...
    private MutableLiveData<List<Article>> healthNews = new MutableLiveData<>();
    ...
    private final String CATEGORY = "health"; // Kategori spesifik untuk ViewModel ini

    public LiveData<List<Article>> getHealthNews() {
        return healthNews;
    }
    ...
    public void fetchHealthNews(boolean isLoadMore) {
        ...
        Log.d("HealthViewModel", "Fetching health news for country: " + Constants.COUNTRY_CODE +
                ", category: " + CATEGORY +
        ...
        call.enqueue(new Callback<NewsResponse>() {
            ...
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (newsResponse.getResults() != null && !newsResponse.getResults().isEmpty()) {
                        ...
                        Log.d("HealthViewModel", "Health news fetched successfully. Total results: " + newsResponse.getTotalResults() +
                                ". Articles fetched in this call: " + newsResponse.getResults().size() +
                                ". Next Page Token: " + nextPageToken);
                    } else {
                        String msg = "Tidak ada berita kesehatan ditemukan untuk " + Constants.COUNTRY_CODE + " (" + Constants.LANGUAGE_CODE + "). Ini mungkin karena batasan API Key gratis atau tidak ada berita yang tersedia.";
                        errorMessage.postValue(msg);
                        if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan tidak ada hasil
                            healthNews.postValue(null);
                        }
                        Log.w("HealthViewModel", msg + " Full response: " + newsResponse.toString());
                    }
                } else {
                    String errorBody = "";
                    try {
                    ...
                    String msg = "Error fetching health news: HTTP " + response.code() + " - " + response.message() + " | " + errorBody;
                    errorMessage.postValue(msg);
                    if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan ada error
                        healthNews.postValue(null);
                    }
                    Log.e("HealthViewModel", msg);
                }
            }
            ...
            public void onFailure(Call<NewsResponse> call, Throwable t) {
                isLoading.postValue(false); // Selesai loading
                String msg = "Kesalahan Jaringan saat mengambil berita kesehatan: " + t.getMessage();
                errorMessage.postValue(msg);
                if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan ada kegagalan
                    healthNews.postValue(null);
                }
                Log.e("HealthViewModel", msg, t);
            }
        });
    }
    ...
    public void resetPagination() {
        nextPageToken = null;
        healthNews.postValue(null); // Kosongkan data yang ada
    ...
    public void loadMoreHealthNews() {
