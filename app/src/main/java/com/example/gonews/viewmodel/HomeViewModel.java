...
import com.example.gonews.model.Article; // Ganti dari 'Article' ke 'NewsArticle' atau nama lain yang Anda gunakan
...
public class HomeViewModel extends ViewModel {
    ...
    // Menggunakan NewsArticle sesuai dengan model NewsData.io yang baru
    private MutableLiveData<List<Article>> latestNews = new MutableLiveData<>();
    ...
    public LiveData<List<Article>> getLatestNews() {
        return latestNews;
    }
    ...
    public void fetchLatestNews(boolean isLoadMore) {
        ...
        Log.d("HomeViewModel", "Fetching latest news for country: " + Constants.COUNTRY_CODE +
        ...
        call.enqueue(new Callback<NewsResponse>() {
            ...
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (newsResponse.getResults() != null && !newsResponse.getResults().isEmpty()) {
                        ...
                        Log.d("HomeViewModel", "Latest news fetched successfully. Total results: " + newsResponse.getTotalResults() +
                                ". Articles fetched in this call: " + newsResponse.getResults().size() +
                                ". Next Page Token: " + nextPageToken);
                    } else {
                        String msg = "Tidak ada berita ditemukan untuk " + Constants.COUNTRY_CODE + " (" + Constants.LANGUAGE_CODE + "). Ini mungkin karena batasan API Key gratis atau tidak ada berita yang tersedia.";
                        errorMessage.postValue(msg);
                        if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan tidak ada hasil
                            latestNews.postValue(null);
                        }
                        Log.w("HomeViewModel", msg + " Full response: " + newsResponse.toString());
                    }
                } else {
                    String errorBody = "";
                    try {
                    ...
                    String msg = "Error fetching latest news: HTTP " + response.code() + " - " + response.message() + " | " + errorBody;
                    errorMessage.postValue(msg);
            ...
            public void onFailure(Call<NewsResponse> call, Throwable t) {
                isLoading.postValue(false); // Selesai loading
                String msg = "Kesalahan Jaringan saat mengambil berita terbaru: " + t.getMessage();
                errorMessage.postValue(msg);
                if (!isLoadMore) { // Hanya kosongkan jika ini bukan load more dan ada kegagalan
                    latestNews.postValue(null);
                }
                Log.e("HomeViewModel", msg, t);
            }
        });
    }
    ...
    public void resetPagination() {
        nextPageToken = null;
        latestNews.postValue(null); // Kosongkan data yang ada
